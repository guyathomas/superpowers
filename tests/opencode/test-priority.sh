...
# Verifies that skills are resolved with correct priority: project > personal > workflows
...
# 1. Create in workflows location (lowest priority)
mkdir -p "$HOME/.config/opencode/workflows/skills/priority-test"
cat > "$HOME/.config/opencode/workflows/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Workflows version of priority test skill
---
# Priority Test Skill (Workflows Version)

This is the WORKFLOWS version of the priority test skill.

PRIORITY_MARKER_WORKFLOWS_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/workflows/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Workflows version exists"
else
    echo "  [FAIL] Workflows version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides workflows
echo ""
echo "Test 2: Testing personal > workflows priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides workflows)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_WORKFLOWS_VERSION"; then
    echo "  [FAIL] Workflows version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|workflows" | head -10
fi

# Test 3: Test that project overrides both personal and workflows
echo ""
echo "Test 3: Testing project > personal > workflows priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Workflows version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit workflows: prefix bypasses priority
echo ""
echo "Test 4: Testing workflows: prefix forces workflows version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load workflows:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_WORKFLOWS_VERSION"; then
    echo "  [PASS] workflows: prefix correctly forces workflows version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] workflows: prefix did not force workflows version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
