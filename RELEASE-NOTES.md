...
Workflows for OpenCode now uses OpenCode's native `skill` tool instead of custom `use_skill`/`find_skills` tools. This is a cleaner integration that works with OpenCode's built-in skill discovery.

**Migration required:** Skills must be symlinked to `~/.config/opencode/skills/workflows/` (see updated installation docs).
...
New slash command exposing the deep research skill without code duplication. Invokes `workflows:research` for topic decomposition, parallel subagent research, confidence evaluation, and source-attributed synthesis.

## v4.0.3 (2025-12-26)

### Improvements

**Strengthened using-workflows skill for explicit skill requests**
...
The underlying skills (`workflows:brainstorming`, `workflows:executing-plans`, `workflows:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-workflows` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-workflows`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `workflows:brainstorming`)
...
**Skill priority in using-workflows**
...
- **Rationalizations table** in using-workflows - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-workflows is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > workflows
...
- Optimized workflows bootstrap to eliminate redundant skill execution. The `using-workflows` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-workflows` manually despite already having the content from session start.
...
- Added unified `workflows-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `workflows:skill-name` for workflows skills, `skill-name` for personal
...
- Updated terminology: "Workflows skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/workflows-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/workflows-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core workflows functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-workflows skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-workflows/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-workflows skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-workflows/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `workflows:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `workflows:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `workflows:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `workflows:code-reviewer`
...
- All internal skill references now use `workflows:` namespace prefix
- Updated format: `workflows:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/workflows:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# Workflows v2.0.0 Release Notes

## Overview

Workflows v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([obra/workflows-skills](https://github.com/obra/workflows-skills)). This transforms workflows from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [obra/workflows-skills](https://github.com/obra/workflows-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/workflows/skills/`
...
1. Your old `~/.config/workflows/.git` will be backed up to `~/.config/workflows/.git.bak`
2. Old skills will be backed up to `~/.config/workflows/skills.bak`
3. Fresh clone of obra/workflows-skills will be created at `~/.config/workflows/skills/`

### Removed Features

- **Personal workflows overlay system** - Replaced with git branch workflow
- **setup-personal-workflows hook** - Replaced by initialize-skills.sh
...
- Clones obra/workflows-skills on first run
...
- `WORKFLOWS_SKILLS_ROOT` set to `~/.config/workflows/skills`
...
- Removed obsolete setup-personal-workflows call from session-start
...
- Prominent link to workflows-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in obra/workflows-skills
- `scripts/` directory - Now in obra/workflows-skills/skills/using-skills/
- `hooks/setup-personal-workflows.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/workflows/skills
- `commands/brainstorm.md` - Updated paths to WORKFLOWS_SKILLS_ROOT
- `commands/write-plan.md` - Updated paths to WORKFLOWS_SKILLS_ROOT
- `commands/execute-plan.md` - Updated paths to WORKFLOWS_SKILLS_ROOT
...
- PR #2: Personal workflows overlay system (later replaced)
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/workflows/skills ~/workflows-skills-backup
   ```
...
- Skills repository is now at https://github.com/obra/workflows-skills
...
**Full Changelog:** https://github.com/obra/workflows/compare/dd013f6...main
**Skills Repository:** https://github.com/obra/workflows-skills
